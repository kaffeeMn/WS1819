...
\begin{frame}{Beweis: Stochastische Dominanz}
    ...
    \begin{itemize}
        \only<1>{
            \item Differenz der Kanten $c = (n-1) - s < n-s$
        }\only<2,3,4,5,6>{
            \item Differenz der Kanten 
                    $\textcolor{red}{c} = (n-1) - \textcolor{red}{s} 
                                        < n-\textcolor{blue}{s}$
        }\only<3,4,5,6>{
            \item weitere $c$-Phasen $\Rightarrow$ erwartet mehr $F$-leichte Kanten 
                    in $G$\\
        }\only<4,5,6>{
            \item $s+c = n-1$ Phasen, bzw. $n-1$ Erfolge
        }\only<6>{
    ...
    \begin{overlayarea}{\textwidth}{2cm}
        ...
        \only<2,3,4,5,6>{
            ...
            \draw [ decoration={
                    ...
                    decorate] 
                (c1.west) -- (c3.east) 
                node [pos=0.5,anchor=north,yshift=-0.55cm] {\textcolor{blue}{$c$}}; 
...
\begin{frame}{Beweis: Stochastische Dominanz}
    ...
    \only<3>{
        $X_{np}$ dominiert $X_{sp}$, mit: $$
            n/p > (n-1)/p = E[X_{np}] \geq E[X_{sp}] = s/p
            $$
    }
    \end{overlayarea}
\end{frame}
