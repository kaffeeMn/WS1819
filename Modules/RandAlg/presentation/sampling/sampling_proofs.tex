\begin{frame}{Verschlechtern wir den MSF?}
    ...
    \begin{overlayarea}{\textwidth}{4cm}
        ...
        \begin{itemize}
            ...
            \only<5>{
                \item Ist die betrachtete Kante $F$-leicht und in verschiedenen
                      Komponenten wird sie in $F$ aufgenommen
            }
        \end{itemize}
    \end{overlayarea}
\end{frame}
...
\begin{frame}{Beweis: Stochastische Dominanz}
    ...
    \begin{itemize}
        \only<1>{
            ...
            \item weitere $c$-Phasen, impliziert: mehr $F$-leichte Kanten erwartet 
                    in $G$\\
        }\only<4,5,6>{
            \item $n-1$ Phasen/ Erfolge
        }\only<6>{
            \item Erfolgswahrscheinlichkeit $p$, impliziert: negative 
            Binomialverteilung
        }
    \end{itemize}
    \end{overlayarea}
