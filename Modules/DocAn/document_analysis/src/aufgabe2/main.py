...
def aufgabe2():
    ...
    for cat in brown_categories:
        ...
        # using numpy
        arr_doc_words = np.array([len(brown.words(doc))
                                  for doc in brown.fileids(categories=cat)]).astype(float)
        np_std = np.std(arr_doc_words)
         
        # printing the std for each category       
        print("{}\npython: {}, mean={}\nnp:{}, mean={}".format(cat, std, mean,
                                                               np_std, np.mean(arr_doc_words)))
    ...
    train_samples, train_labels, test_samples, test_labels = cross_validator.corpus_fold(0)
