...
class PythonIntro(object):
    ...
    def datatypes(self, var_tup=None):
        ...
        print('42 in var_tup: {}'.format(b_42))
        print('23 in var_tup: {}'.format(b_23))
    ...
    def sequences(self, seq_start=3, seq_end=7, seq_step=1):
        ...
        anzahl_el = len(test_list)
        erste = test_list[0]
        letzte = test_list[-1]
        vor_und_letzte = test_list[-2:]
        erste_drittel = test_list[:int(anzahl_el/3)]
        zweite_drittel = test_list[int(anzahl_el/3):2*int(anzahl_el/3)]
        letzte_drittel = test_list[2*int(anzahl_el/3):]
    
    
        num_list = [anzahl_el, erste, letzte, vor_und_letzte, erste_drittel,
                    zweite_drittel, letzte_drittel]
       ...
       # raise NotImplementedError('Implement me')
        ...
        str_list = []
        for idx, el in enumerate(test_list):
            str_list.append('%d: %d ' % (idx, el))#'{}: {}'.format(idx, el))
            print(str_list[idx])
      #  raise NotImplementedError('Implement me')
    ...
    def sequences_complex(self, test_seq=(1, 1.0, '1.0')):
        ...
        #type_list = ["<{}: {}>" for t,v in zip([type(e) for e in test_seq]), test_seq]
        type_str = '['
        for i, el in enumerate(test_seq):
            type_str += " <" + str(type(el).__name__) + ", " \
                         + str(el) + ">" #"<{}: {}>".format(type(el), el))
            if i != len(test_seq)-1:
                type_str += ','
        type_str += ' ]'
        print(type_str)
    ...
    #    raise NotImplementedError('Implement me')
        ...
        type_strs = []
        for i, el in enumerate(test_seq):
            type_strs.append('<%s, %s>' % (type(el).__name__, el))
        type_str2 = ' '.join(['[', ', '.join(type_strs), ']'])
        print(type_str2) 
        #raise NotImplementedError('Implement me')
        return type_str2
    ...
    def list_comprehension(self, test_seq=(1, 1.0, '1.0')):
        ...
        type_names = [type(el).__name__ for el in test_seq]
        zipped = zip(type_names, test_seq)
        correct_form = ['<%s, %s>' % item for item in zipped]
        
        print(' '.join(['[ ', ', '.join(correct_form), ']']))
        ...
        return (el for el in correct_form)
    ...
    def dictionaries(self, rand_list=None):
        ...
        int_vals = [int(round(v)) for v in rand_list]
...
class NumPyIntro(object):
    ...
    def arrays(self, test_arr=None):
        ...
        arr = np.arange(start = 5, stop = 17).reshape(4,3)
